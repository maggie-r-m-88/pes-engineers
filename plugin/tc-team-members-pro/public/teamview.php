  ...
  foreach ( $members as $member ) {
          ...
          $output .= '<div class="tc_overlay" style="background-color:'.$rgba_bg_color.';color:'.$text_color.';">';
           ...
           if(!empty($member['job_role'])){
            $output .= '<h4 class="tc-team-title-sm">'.esc_attr( $member['job_role'] ).'</h4>';
             ...
             $output .= '<a class="tc_pu_views open-popup-link" data-mfp-src="#tc_pu_view_'.$members_loop.'" href="javascript:void(0);" data-effect="">
    ...
    $output .= '<h4 class="tc-team-title-sm">'.esc_attr( $member['job_role'] ).'</h4>';
      ...
      if(!empty($member['email_id'])){
       $output .= '<p class="tc-team-email-id"> <i class="fa fa-envelope"></i> '.esc_attr( $member['email_id'] ).'</p>';

       }
       if(!empty($member['tel_number'])){
        $output .= '<p class="tc-team-tel-number"> <i class="fa fa-phone"></i>'.esc_attr( $member['tel_number'] ).'</p>';

        }

 // start pop up -->
$output .= '<div id="tc_pu_view_'.$members_loop.'" class="white-popup mfp-hide mfp-with-anim">';

         $output .= '<div class="tc_pu_left_block">';


          $output .= '<img class="tc-pu-team-img" src="'.$member['member_image'].'" />';
...
$output.=$qv_output;
   ...
   $output.='</div>';
   $output.='<div class="tc_pu_right_block">';


       $output.='<h1  class="tc-pu-team-title-lg">'.$member['full_name'].'</h1>';
       $output.='<h2 class="tc-pu-team-title-sm">'.$member['job_role'].'</h2>';
       $output.=wpautop($member['description']);


$output.='</div>';  // tc_qv_left_block  -->
   $output.='</div>';
        ...
        $output .= '<div class="tc_team-member2  '.$tc_column.'  tc_text-center">';
            ...
            $output .= '<div class="tc_member-thumb2-img tc_pu_box">';
              $output .= '<div class="tc_member-thumb2-box '.$hover.'">';
               if(!empty($member['member_image'])){
                $output .= '<a class="tc_pu_views open-popup-link" data-mfp-src="#tc_pu_view_'.$members_loop.'" href="javascript:void(0);" data-effect=""><img src="' . esc_attr( $member['member_image'] ) . '" alt="' . esc_attr( $member['full_name'] ) . '" /> </a>';
...
$output.=$qv_output;
   ...
   $output.='<div class="tc_pu_right_block">';
       ...
       $output.='<h1  class="tc-pu-team-title-lg">'.$member['full_name'].'</h1>';
       $output.='<h2 class="tc-pu-team-title-sm">'.$member['job_role'].'</h2>';
       //$output.='<p  class="tc-pu-meam-p">'.$member['description']. '</p>';
       $output.=wpautop($member['description']);
  ...
  foreach ( $members as $member ) {
    ...
    $output .= '<div class="tc_team-members3 tc_teambox">';
       $output .= '<div class="tc_member-thumb3-img">';
        ...
        $output .='<div class="tc-contact-box-3">';
...
$output.=$qv_output;
   ...
   $output.='<div class="tc_pu_right_block">';
       ...
       $output.='<h1  class="tc-pu-team-title-lg">'.$member['full_name'].'</h1>';
       $output.='<h2 class="tc-pu-team-title-sm">'.$member['job_role'].'</h2>';
       //$output.='<p  class="tc-pu-meam-p">'.$member['description']. '</p>'; version 3.5
       $output.=wpautop($member['description']); // version 3.6
  ...
  foreach ( $members as $member ) {
    ...
    $output .= '<div class="tc_team-members4 tc_teambox">';
         ...
         $output .= '<div class="tc_member-thumb4-img tc_pu_box">';
            ...
            if(!empty($member['member_image'])){
              $output .= '<a class="tc_pu_views open-popup-link"  data-mfp-src="#tc_pu_view_'.$members_loop.'" href="javascript:void(0);" data-effect=""> <img class="tc-member-4" src="' . esc_attr( $member['member_image'] ) . '" alt="' . esc_attr( $member['full_name'] ) . '" /></a>';
             }
           ...
           if(!empty($member['job_role'])){
            $output .= '<h2 class="tc-team-title-sm6">'.esc_attr( $member['job_role'] ).'</h2>';
             ...
             if(!empty($member['description'])){
              ...
              $output .= '<div class="tc-pu-box"> <a class="tc_pu_views open-popup-link" data-mfp-src="#tc_pu_view_'.$members_loop.'" href="javascript:void(0);" data-effect="">
