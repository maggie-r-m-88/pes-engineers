...
if ($layout==1) {
  ...
  $layoutnum=1;
  $members_loop=(int)($layoutnum.$teamid);
  foreach ( $members as $member ) {
    $members_loop ++;
    $output .= '<div class="tc_team-member1 tc_member-col-single tc_text-center">';
          ...
          $output .= '<div class="tc_overlay" style="background-color:'.$rgba_bg_color.';color:'.$text_color.';">';
           ...
           if(!empty($member['job_role'])){
            $output .= '<h4 class="tc-team-title-sm">'.esc_attr( $member['job_role'] ).'</h4>';
             ...
             $output .= '<a class="tc_pu_views open-popup-link" href="#tc_pu_view_'.$members_loop.'" data-effect="mfp-zoom-in"><span class=" tc-meam-p tc-pu"><i class="fa fa-eye-slash" aria-hidden="true"></i></span></a>';

              // $output .= '<a class="tc_pu_views open-popup-link" href="#tc_pu_view_'.$members_loop.'" data-effect="mfp-zoom-in"><h3 class="tc-team-title-lg"><p class="tc-meam-p">'.substr($member['description'],0,80).'</p></a>';
            }
            if (! $hidesocialicon) {
...
// Start Layout
if ($layout==2) {
  $output .='<div class="wrap  clearfix">';
  $layoutnum=2;
  $members_loop=(int)($layoutnum.$teamid);
  foreach ( $members as $member ) {
  $members_loop ++;
    $output .= '<div class="tc_team-members2-box tc_teambox">';
        $output .= '<div class="tc_team-member2  tc_member-col-single2  tc_text-center">';
            ...
            $output .= '<h3  class="tc-team-title-lg">'. esc_attr( $member['full_name'] ).'</h3>';
             ...
             if(!empty($member['job_role'])){
              $output .= '<h4 class="tc-team-title-sm">'.esc_attr( $member['job_role'] ).'</h4>';
               ...
               if(!empty($member['description'])){
              $output .= '<p  class="tc-meam-p">'. substr($member['description'],0,80) .'</p>';
               }
              if (! $hidesocialicon) {
          $output .= '<ul class="tc_social-links">';
...
$output .='<div class="wrap  clearfix">';
$layoutnum=4;
 //$members_loop=(int)($layoutnum.$teamid);rand(5, 15);
           ...
           if(!empty($member['job_role'])){
            $output .= '<h2 class="tc-team-title-sm6">'.esc_attr( $member['job_role'] ).'</h2>';
             ...
             if(!empty($member['description'])){
              ...
              $output .= '<div class="tc-pu-box"> <a class="tc_pu_views open-popup-link" href="#tc_pu_view_'.$members_loop.'" data-effect="mfp-zoom-in"><span class=" tc-meam-p tc-pu"><i class="fa fa-eye-slash" aria-hidden="true"></i></span></a></div>';
            }
            if (! $hidesocialicon) {
            $output .= '<ul class="tc_social-links">';
...
if ($layout==7) {
  ...
  foreach ( $members as $member ) {
    ...
    $output .= '<div class="tc_team-member7 tc_member-col-single7  tc_text-center">';
